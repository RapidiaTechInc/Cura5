...
for package in ${troublesome_packages[@]}; do
    ...
    package_contains_at_symbol=$(echo $package|grep -c "@") #if no @ symbol in package then this will be 0 else number > 0
    echo "package contains at $package_contains_at_symbol"
    if [[ package_contains_at_symbol -eq 0 ]]; then
