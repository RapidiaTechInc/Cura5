...
class SupportEraser(Tool):
    ...
    def event(self, event):
        ...
        if (
            ...
            and MouseEvent.LeftButton in event.buttons
            ...
            if self._selection_pass is None:
                ...
                self._selection_pass = (
                    Application.getInstance().getRenderer().getRenderPass("selection")
                )
            ...
            if node_stack:
                if node_stack.getProperty(
                    "anti_overhang_mesh", "value"
                ) or node_stack.getProperty("support_modifier_mesh", "value"):
                    self._removeEraserMesh(picked_node)
                    return

                elif (
                    node_stack.getProperty("support_mesh", "value")
                    or node_stack.getProperty("infill_mesh", "value")
                    or node_stack.getProperty("cutting_mesh", "value")
                    or node_stack.getProperty("support_modifier_mesh", "value")
                ):
            ...
            picking_pass = PickingPass(
                active_camera.getViewportWidth(), active_camera.getViewportHeight()
            )
            picking_pass.render()
    ...
    def _createEraserMesh(self, parent: CuraSceneNode, position: Vector):
        ...
        op.addOperation(
            AddSceneNodeOperation(node, self._controller.getScene().getRoot())
        )
    ...
    def _updateEnabled(self):
        ...
        if global_container_stack:
            plugin_enabled = global_container_stack.getProperty(
                "anti_overhang_mesh", "enabled"
            )

        CuraApplication.getInstance().getController().toolEnabledChanged.emit(
            self._plugin_id, plugin_enabled
        )
